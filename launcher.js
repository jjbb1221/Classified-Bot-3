...
if (shardCount === 'auto') {
  Util.fetchRecommendedShards(token, guildsPerShard).then(count => {
    ...
    `);
    const manager = new ShardingManager(`${__dirname}/Kosh.js`, { totalShards: parseInt(count.toFixed(0)), token, respawn: autoRespawn })
  ...
  console.log(stripIndents`
    ...
    Estimated launch time: ${moment.duration(time).format(' m [mins], s [secs]')}
  `);
  const manager = new ShardingManager(`${__dirname}/Kosh.js`, { totalShards: shardCount, token, respawn: autoRespawn })
    .on('launch', shard => console.log(`Attempting to launch shard ${shard.id + 1} | PID: ${shard.process.pid}.`));
